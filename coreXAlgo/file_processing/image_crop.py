...
class TaggedImageCrop:
    ...
    Args:
        ...
        stride (int): 滑动步长
        separate_images_xml (bool): 是否将图片和XML分开保存
        generate_ok_xml (bool): 是否为OK图生成XML文件
        generate_nofound_tag (bool): 是否为OK图生成nofound标签
    ...
    Example:
        ...
        >>>     crop_size=640,                    # 裁剪尺寸
        >>>     stride=320,                       # 滑动步长
        >>>     separate_images_xml=True,         # 分开保存图片和XML
        >>>     generate_ok_xml=True,             # 为OK图生成XML
        >>>     generate_nofound_tag=True,        # 为OK图生成nofound标签
        >>>     verbose=True                      # 显示详细日志
        >>> )
    ...
    def __init__(self, retrain_no_detect=False, separate_ok_ng=False, save_only_ok=False, save_dir=None, target_size=None, crop_size=640,
                 stride=320, separate_images_xml=True, generate_ok_xml=True, generate_nofound_tag=False, verbose=False):
        """
        初始化图像裁剪处理器
        ...
        Args:
            ...
            separate_ok_ng (bool): 是否将OK和NG图像区分保存
                - True: OK图像保存到ok_images目录，NG图像保存到ng_images目录
                - False: 所有图像都保存到ng_images目录
            ...
            generate_nofound_tag (bool): 是否为OK图生成nofound标签
                - True: 为OK图生成XML文件时添加nofound标签，标签位置在图片正中心，宽度为50
                - False: 为OK图生成XML文件时不添加标签
            verbose (bool): 是否显示详细日志信息

        注意：目录创建逻辑已优化，只有在实际保存文件时才会创建对应的目录，避免创建空文件夹。
        """
        ...
        self.stride = stride
        self.separate_images_xml = separate_images_xml
        ...
        # 初始化保存目录路径
        if save_dir:
            # 清理路径，移除可能的换行符和错误的分隔符
            save_dir = save_dir.strip()
            # 替换错误的分隔符为正确的Windows路径分隔符
            ...
            if self.save_only_ok:
                # 如果只保存OK图片
                if self.separate_images_xml:
                    # 分开保存：图片到ok_images，XML到ok_xmls
                    self.save_ok_img_dir = os.path.join(save_dir, "ok_images")
                    self.save_ok_xml_dir = os.path.join(save_dir, "ok_xmls")
                else:
                    # 同一目录保存：图片和XML都到ok_images
                    self.save_ok_img_dir = os.path.join(save_dir, "ok_images")
                    self.save_ok_xml_dir = self.save_ok_img_dir
                self.save_img_dir = None
                self.save_xml_dir = None
            else:
                # 正常模式
                if self.separate_images_xml:
                    # 分开保存：图片到ng_images，XML到ng_xmls
                    self.save_img_dir = os.path.join(save_dir, "ng_images")
                    self.save_xml_dir = os.path.join(save_dir, "ng_xmls")
                else:
                    # 同一目录保存：图片和XML都到ng_images
                    self.save_img_dir = os.path.join(save_dir, "ng_images")
                    self.save_xml_dir = self.save_img_dir

                if self.retrain_no_detect and self.separate_ok_ng:
                    if self.separate_images_xml:
                        # 分开保存：OK图片到ok_images，OK XML到ok_xmls
                        self.save_ok_img_dir = os.path.join(save_dir, "ok_images")
                        self.save_ok_xml_dir = os.path.join(save_dir, "ok_xmls")
                    else:
                        # 同一目录保存：OK图片和XML都到ok_images
                        self.save_ok_img_dir = os.path.join(save_dir, "ok_images")
                        self.save_ok_xml_dir = self.save_ok_img_dir
                else:
                    self.save_ok_img_dir = None
                    self.save_ok_xml_dir = None
        else:
            self.save_dir = None
            self.save_img_dir = None
            self.save_ok_img_dir = None
    ...
    def _is_defect_in_crop(self, defect_box, crop_box, defect_name):
        ...
        if self.verbose and defect_name.endswith('U4U'):
            self.logger.debug(
                f"U4U缺陷检查: 交集面积={intersect_area}, 缺陷面积={defect_area}, "
        ...
        if defect_name.endswith('U4U'):
            ...
            if intersect_area > 0.1 * crop_area:  # 交集占裁剪块10%以上
                return True, (
                    inter_xmin - crop_box[0], 
                    inter_ymin - crop_box[1],
                    inter_xmax - crop_box[0], 
                    inter_ymax - crop_box[1]
                )
    ...
    def _save(self, crop_image, cropped_labels, base_filename, crop_index):
        ...
        if self.save_only_ok and not has_labels:
            ...
            xml_save_path = self.save_ok_xml_dir
            save_type = "OK(only)"
        else:
            # 正常模式
            if not has_labels and self.retrain_no_detect and self.separate_ok_ng and self.save_ok_img_dir:
                # 无标签 + 保留无标签图片 + 分开保存 -> 保存到ok_images和ok_xmls
                img_save_path = self.save_ok_img_dir
                xml_save_path = self.save_ok_xml_dir
                save_type = "OK"
            else:
                # 有标签或不分开保存 -> 保存到ng_images和ng_xmls
                img_save_path = self.save_img_dir
                xml_save_path = self.save_xml_dir
        ...
        # 保存XML文件 - 根据generate_ok_xml参数决定是否为OK图生成XML
        if has_labels or self.generate_ok_xml:
            xml_filename = f"{base_filename}_{crop_index}.xml"
            xml_full_path = os.path.join(xml_save_path, xml_filename)
            ...
            try:
                ...
                # 添加标签（如果有）
                if has_labels and cropped_labels:
                ...
                # 为OK图添加nofound标签
                elif not has_labels and self.generate_nofound_tag:
                    # 计算图片中心坐标
                    img_width, img_height = crop_image.shape[1], crop_image.shape[0]
                    center_x = img_width // 2
                    center_y = img_height // 2
                    # 设置标签宽度为50，计算边界框
                    box_width = 50
                    xmin = max(0, center_x - box_width // 2)
                    ymin = max(0, center_y - box_width // 2)
                    xmax = min(img_width, center_x + box_width // 2)
                    ymax = min(img_height, center_y + box_width // 2)
                    # 添加nofound标签
                    voc_ann.add_object(
                        name="nofound",
                ...
                if self.verbose:
                    if has_labels:
                        self.logger.debug(f"保存XML: {xml_filename} 包含 {len(cropped_labels)} 个目标 -> {os.path.basename(xml_save_path)}")
                    else:
                        self.logger.debug(f"保存XML: {xml_filename} (无目标) -> {os.path.basename(xml_save_path)}")
            except Exception as e:
    ...
    def crop_image_and_labels(self, image_path, xml_path):
        ...
        Example:
            ...
            >>>
            >>> # 处理后的文件结构 (separate_images_xml=True):
            >>> # output_crops/
            >>> #   ├── ng_images/      # 包含目标的裁剪图像
            >>> #   ├── ng_xmls/        # 包含目标的XML标注
            >>> #   ├── ok_images/      # 无目标的裁剪图像（如果retrain_no_detect=True）
            >>> #   └── ok_xmls/        # 无目标的XML标注（如果generate_ok_xml=True）
            >>>
            >>> # 处理后的文件结构 (separate_images_xml=False):
            >>> # output_crops/
            >>> #   ├── ng_images/      # 包含目标的裁剪图像和XML
            >>> #   └── ok_images/      # 无目标的裁剪图像和XML（如果retrain_no_detect=True）
            >>>
            >>> 注意：只有当实际有文件需要保存时，才会创建对应的目录，避免创建空文件夹。
        """
        # 读取图像
        image, original_size = self._process_image(image_path)
        if image is None:
            return {'total_crops': 0, 'ng_crops': 0, 'ok_crops': 0}
        ...
        if self.verbose:
            self.logger.info(
                f"处理完成: {base_name} - 总裁剪块: {stats['total_crops']}, "
...
def batch_multithreaded_image_cropping(img_path_list, processor, max_workers=10, verbose=False):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # 检查sys.stdout是否为None，避免tqdm写入错误
        import sys
        disable_tqdm = sys.stdout is None
